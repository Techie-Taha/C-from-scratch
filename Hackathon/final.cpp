    ...
    for(int i = 0; i < updated_cipherText.size(); i++){
        if(isalpha(updated_cipherText.at(i))){
            ...
            if(toVoidrepetition.find(x) == string::npos){
                int count = 0;
                for(int j = 0; j < updated_cipherText.size(); j++){ // chech to see whether x is already in toVoidrepetition, if not, second loop starts
...
// Step 2 and 3 : the correct original (decrypted) English plaintext and best key
    ...
    bool firstKey = true; // goal is to loop though every possuble keys, haven't precoessed any keys yet
// // Boolean works better since we have to check each keys. And using the concept of score, when firstKey = false, we can compare it
    // we going to run a loop through the vector keys
    for(int k = 0; k < keys.size(); k++){
        string currentKey = keys.at(k); // basically we have a container that hold whatever current key being tested
...
// formula : 
            // just a container to store it;

            int val = (updated_cipherText.at(i) - 'A' - (key - 'a') + 26) % 26;
...
accessed: Oct. 2025
 -------- */
        ...
        int score = currentScore > highestScore;
        if(firstKey || score){
...
now highest score is 120, lets say utter, and correcttext will be decrypte text with "utter"
and then our first key becomes false;
*/
        }

    }
    string finalText; // empty string which will store the final decrypted tex 
...
// we will list the decrypted text with spaces with 5 character, just to read it better
         if(i % 4 == 3){
            finalText += ' ';
         }
    }
